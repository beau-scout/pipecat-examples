...
- Hailey's voice (TTS) is provider-switchable via `TTS_PROVIDER` in `bot.py`: `cartesia` (default, lowest latency/cost) or `elevenlabs` (higher cloning fidelity). Each reads its own key + voice ID (`CARTESIA_VOICE_ID` / `ELEVENLABS_VOICE_ID`); ElevenLabs defaults to the `eleven_flash_v2_5` model for phone-grade latency. The flag exists to A/B a clone of the operator's real voice on a live call.
