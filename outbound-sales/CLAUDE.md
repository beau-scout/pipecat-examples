...
- **Resume is in-flight-safe.** A call gets a provisional `in_progress` row the moment its bot starts (in `handle_dial_out_request`), not only when it reports an outcome at call end. Without this, stopping and restarting a campaign mid-call re-dialed every in-flight number (the resume logic still saw it as "never called") — double-dialing the same school. The bot's terminal outcome overwrites the provisional row (`handle_call_result` replaces an `in_progress` row but still keeps the first *terminal* row per call_id). The dialer skips numbers whose `in_progress` row is within `IN_PROGRESS_COOLDOWN_SECS` (default 900s); past that, a stuck `in_progress` row (bot died without reporting) becomes retry-eligible again. Batch size is `DIALER_BATCH_SIZE` / `--batch-size` (default 5).
