...
- **Prompt caching is on (`enable_prompt_caching=True`) and the model choice depends on it.** Pipecat's Anthropic adapter caches by putting a `cache_control: ephemeral` breakpoint on the last two user messages; Anthropic then caches the whole prefix before each breakpoint — tools + system + earlier turns — so the large static system prompt re-reads at ~10% cost on every turn after the first. That's the biggest per-call cost lever, and it's verified in production (per-call summary logs `cache_read` tokens; healthy calls show 50–100% read). **The catch:** Anthropic only caches a prefix above a per-model minimum — 2048 tokens for Sonnet 4.6, but **4096** for Haiku 4.5 and every Opus tier. Our static prefix (system prompt ~2.2k tokens + the two tool schemas ≈ 2.6k) clears Sonnet 4.6's floor but is *below* Haiku/Opus's, so pointing `ANTHROPIC_MODEL` at `claude-haiku-4-5` or an Opus would **silently turn caching off** (no error — each turn just re-pays full price for the system prompt). Sonnet 4.6 is the default for exactly this reason; if you change models, watch the logged `cache_read` % to confirm caching still engages.
