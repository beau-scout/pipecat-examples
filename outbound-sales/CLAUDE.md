...
- Hailey's voice (TTS) is provider-switchable via `TTS_PROVIDER` in `bot.py`: `elevenlabs` (default, most natural; requires `ELEVENLABS_API_KEY`) or `cartesia` (lowest latency/cost). Each reads its own key + voice ID (`ELEVENLABS_VOICE_ID` / `CARTESIA_VOICE_ID`); ElevenLabs defaults to the `eleven_turbo_v2_5` model (best quality/latency balance for phone). Set a voice ID to use a library voice or a clone of the operator's real voice.
